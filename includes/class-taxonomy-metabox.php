...
class EFramework_Taxonomy_Metabox
{
    /**
     * Version
     * @var string
    ...
    function __construct( $redux )
    {
        ...
        do_action( 'cms_taxonomy_metabox_register', $this );
    ...
    function admin_init()
    {
        ...
        if ( 'term.php' == $pagenow )
        {
            $this->notices = get_transient( 'cms-taxonomy-metabox-transients' );

            // If the transient exists then we override it with an empty value
            if ( false !== $this->notices )
            {
                set_transient( 'cms-taxonomy-metabox-transients', '' );
            }
            else
            {
    ...
    function add_section( $taxonomy, $section = array() )
    {
        ...
        foreach ( $section['fields'] as $fkey => $field )
        {
            ...
            {
                trigger_error( sprintf( esc_html__( 'The field with id %1$s for taxonomy %2$s is already registered.', CMS_TEXT_DOMAIN ), esc_html( $field['id'], CMS_TEXT_DOMAIN ), esc_html( $taxonomy, CMS_TEXT_DOMAIN) ) );
    ...
    function panel_scripts()
    {
        ...
        wp_enqueue_style( 'cms-metabox', abtheme()->path( 'APP_URL' ) . '/assets/css/metabox' . Redux_Functions::isMin() . '.css', array(), self::$version, 'all' );
        wp_enqueue_script( 'cms-metabox', abtheme()->path( 'APP_URL' ) . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array( 'jquery', 'redux-js' ), self::$version, 'all' );
    ...
    protected function generate_output_css( $taxonomy, $options )
    {
        ...
        echo '<style type="text/css" id="cms-' . $taxonomy . '-dynamic-css">' . $redux->outputCSS . '</style>';
    }
}
